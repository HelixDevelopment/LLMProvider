...
func TestNewProvider(t *testing.T) {
	provider := NewStepfunProvider("test-api-key", "", "")
	assert.NotNil(t, provider)
	assert.Equal(t, "test-api-key", provider.apiKey)
	assert.Equal(t, "https://api.stepfun.com/v1/chat/completions", provider.baseURL)
	assert.Equal(t, "step-1.5v-mini", provider.model)
}

func TestNewProviderWithCustomURL(t *testing.T) {
	customURL := "https://custom.api.com/v1/chat/completions"
	provider := NewStepfunProvider("test-key", customURL, "custom-model")
...
func TestNewProviderWithRetry(t *testing.T) {
	...
	provider := NewStepfunProviderWithRetry("test-key", "", "", retryConfig)
...
func TestComplete(t *testing.T) {
	...
	provider := NewStepfunProvider("test-api-key", server.URL, "")
	...
	assert.Equal(t, "stepfun", resp.ProviderID)
}
...
func TestCompleteWithError(t *testing.T) {
	...
	provider := NewStepfunProvider("invalid-key", server.URL, "")
...
func TestCompleteStream(t *testing.T) {
	...
	provider := NewStepfunProvider("test-key", server.URL, "")
...
func TestGetCapabilities(t *testing.T) {
	provider := NewStepfunProvider("test-key", "", "")
	...
	assert.Contains(t, caps.SupportedFeatures, "gui_grounding")
	assert.True(t, caps.SupportsStreaming)
	assert.True(t, caps.SupportsVision)
	assert.Equal(t, StepfunMaxOutput, caps.Limits.MaxTokens)
	assert.Equal(t, StepfunMaxContext, caps.Limits.MaxInputLength)
}
...
func TestValidateConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider := NewStepfunProvider(tt.apiKey, "", "")
...
func BenchmarkStepfunProvider_ConvertRequest(b *testing.B) {
	provider := NewStepfunProvider("test-key", "", "")
	req := &models.LLMRequest{
		ID:     "bench-request",
...
func BenchmarkStepfunProvider_ConvertResponse(b *testing.B) {
	provider := NewStepfunProvider("test-key", "", "")
	req := &models.LLMRequest{ID: "bench-request"}
	startTime := time.Now()
	sResp := &StepfunResponse{
		ID:    "resp-456",
		Model: "step-1.5v-mini",
		Choices: []StepfunChoice{
			{
				Index:        0,
				Message:      StepfunMessage{Role: "assistant", Content: "Benchmark response content"},
				FinishReason: "stop",
			},
		},
		Usage: StepfunUsage{
