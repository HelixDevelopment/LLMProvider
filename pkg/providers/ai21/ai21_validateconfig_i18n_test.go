...
// non-English string for the ValidateConfig API-key message ID. Mocks
// are permitted in unit tests per CONST-050(A).
...
// TestAI21ValidateConfig_I18nSeam_Localized is the POSITIVE half of the
// round-425 CONST-046 paired mutation: with a real Translator wired the
...
func TestAI21ValidateConfig_I18nSeam_Localized(t *testing.T) {
	defer i18n.SetTranslator(nil)
	i18n.SetTranslator(vcLocaleTranslator{})

	p := NewProvider("", "https://api.ai21.com", "jamba-mini")
	...
	if strings.Contains(errs[0], "API key is required") {
		t.Fatalf("ValidateConfig emitted hardcoded English literal %q — CONST-046 round-425 migration regressed", errs[0])
	}
...
// TestAI21ValidateConfig_I18nSeam_NoopFallback is the NEGATIVE half:
...
func TestAI21ValidateConfig_I18nSeam_NoopFallback(t *testing.T) {
